...
describe('ReadingBookListClient component', () => {
  const expectedBooks = [bookWithImage, bookWithoutImage]
  const expectedReadingBooks = [lendingHistory1, lendingHistory2]

  it('返却期限と本の一覧が表示される', () => {
    render(<ReadingBookListClient books={expectedBooks} readingBooks={expectedReadingBooks} />)
  ...
  it('本がない場合は「該当の書籍はありません」というメッセージが表示される', () => {
    render(<ReadingBookListClient books={[]} readingBooks={[]} />)

    expect(screen.getByText('該当の書籍はありません')).toBeInTheDocument()
  })

  it('期限が過ぎている場合、返却期限は赤太字で表示される', () => {
